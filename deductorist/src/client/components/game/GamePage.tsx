...
export const GamePage: React.FC = () => {
  ...
  const { isDesktop, clueIconSize, hasMouse, requiredDrawerHeight } = useBoardLayout(puzzle, viewport, activeMobileTab);
  ...
  const triggerSave = useCallback(() => {
    ...
    pendingSyncTimerRef.current = setTimeout(() => {
       ...
       fetch('/api/game/state/sync', {
          ...
          keepalive: true
       }).catch((err) => console.warn('[GamePage] Sync failed', err));
    }, 3000);
